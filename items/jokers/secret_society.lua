SMODS.Joker {
    ...
    loc_vars = function(self, info_queue, card)
        ...
        return { vars = { (nom_sum - SMODS.Ranks['Ace'].nominal) * 2, (nom_sum - SMODS.Ranks['2'].nominal) * 2 } }
    end,
}
...
function Card:get_chip_bonus()
    if next(SMODS.find_card('j_mxms_secret_society')) and not self.config.center.replace_base_card and not self.ability.extra_enhancement then
        return ((Maximus.get_nominal_sum() - self.base.nominal) * 2) + self.ability.bonus + (self.ability.perma_bonus or 0)
    end
    return gcb(self)
end